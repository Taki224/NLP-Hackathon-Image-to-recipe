...
- [X] Update `FoodRecipeDataset` instantiation to load the new `paired_dataset.json`.
- [X] Create DataLoader for training on the combined dataset.
- [X] Update the `text_adapter` and tokenizer logic to use LongCLIP's tokenizer for full recipe text (Title + Ingredients + Instructions).

### 2.2 Model Architecture Experiments
- [X] Use LongCLIP as the base model to support full recipe text (title + ingredients + instructions).
- [X] Verify full recipe text (title + ingredients + instructions) fits within LongCLIP's 248-token limit; truncate instructions if needed, prioritising title and ingredients
- [X] Re-introduce unfreezing the last few layers of the LongCLIP model (image and text encoders) since the dataset has successfully scaled to 74k high-quality pairs.
- [X] Experiment with lightweight adapter architectures (e.g., a bottleneck MLP like 512 -> 64 -> 512) or a TIP-Adapter and compare performance against the unfrozen base layers.
- [X] Experiment with generating hard positive and negative pairs for training to improve model discrimination between similar dishes (e.g., Goulash vs Mediterranean soup).
- [X] Experiment with training on a larger but lower-quality dataset (e.g., bypassing VLM validation or lowering SigLIP thresholds) to baseline if raw volume outperforms our filtered 74k high-quality data.

### 2.3 Training Loop & Metrics
- [X] Track and log training loss throughout training.
- [X] Save model checkpoints at regular intervals (e.g., every N epochs or when loss improves).
- [X] Log and plot training loss progression.

### 2.4 Post-Training Tasks
- [X] After training completes, re-run the Step 10 `retrieve()` sanity check with a few sample images to confirm the new model and updated index produce sensible results before hackathon day.
- [X] Generate the recipe retrieval `.npy` index for all 2 million recipes after training is complete (batch loading to avoid RAM crashes).
- [X] Update `retrieve()` function to accept recipe IDs from index and perform metadata lookup to return full recipe text (title, ingredients, instructions).
- [X] Update `/retrieve` API endpoint in `ui/src/app.py` to include full recipe instructions in the JSON response alongside similarity scores.
- [X] Update the UI (`ui/static/index.html`) to display full recipe instructions in recipe cards below the ingredients list.
